...
BEGIN
  ...
  -- Check if this is the first attempt for this PASSAGE (not just game type)
  SELECT COUNT(*) = 0 INTO v_is_first_attempt
  FROM game_attempts
  WHERE user_id = p_user_id 
    AND passage_id = p_passage_id;

  -- Get the previous best score for this game type in this unit
  ...
  -- XP calculation: Only award XP on first attempt of a passage OR if score improved overall
  IF v_is_first_attempt THEN
    -- First attempt of this passage: award base XP from score + time bonus
  ...
  ELSIF v_score > v_previous_best_score THEN
    -- Improvement over previous best: award bonus XP for the score improvement
  ...
  -- Insert game attempt WITH passage_id
