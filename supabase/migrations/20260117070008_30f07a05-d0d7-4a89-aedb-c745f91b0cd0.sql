-- Drop old functions and recreate with proper game_id usage
DROP FUNCTION IF EXISTS public.validate_dictation_game_submission(uuid, uuid, text, jsonb, integer);

-- Recreate function that accepts game_type string but uses game_id internally
...
BEGIN
  ...
  -- Update user_progress for this specific game using game_id
  ...
  ELSE
    ...
    VALUES (p_user_id, p_unit_id, v_game_id, v_score, v_game_xp, p_time_spent_seconds, 1, v_is_perfect);
  END IF;
