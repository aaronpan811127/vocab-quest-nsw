-- Fix: Add bounds checking for answer_index in validate_game_submission
-- This prevents errors when user provides invalid array indices
...
BEGIN
  ...
  -- Process each answer - FIXED: use game_id instead of game_type
  ...
  IF v_existing_progress IS NULL THEN
    ...
    VALUES (p_user_id, p_unit_id, p_game_id, v_score, v_game_xp, 1, true, p_time_spent_seconds);
  ELSE
    UPDATE user_progress
    SET 
