...
-- Fix validate_game_submission: derive test_type from the unit, not the user's default
...
BEGIN
  ...
  -- Get test type from the unit being played (NOT the user's default)
  SELECT test_type_id INTO v_test_type_id
  FROM units WHERE id = p_unit_id;
  
  -- Fallback: if unit has no test type, use user's default
  ...
  -- Only mark completed when score is perfect (100%)
  ...
  RETURN jsonb_build_object(
    ...
    'incorrect_question_ids', v_incorrect_question_ids
  );
END;
$$;

-- Fix validate_dictation_game_submission: derive test_type from the unit, not the user's default
...
BEGIN
  ...
  -- Look up the game_id from the game_type
  ...
  SELECT ttg.contributes_to_xp INTO v_contributes_to_xp
  FROM test_type_games ttg
  WHERE ttg.test_type_id = v_test_type_id AND ttg.game_id = v_game_id;
  ...
  -- Insert game attempt using game_id
