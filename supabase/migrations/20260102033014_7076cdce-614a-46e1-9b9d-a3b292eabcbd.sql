-- Restrict reading_passages to authenticated users only
DROP POLICY IF EXISTS "Anyone can view passages" ON public.reading_passages;
...
-- Restrict units to authenticated users only  
...
-- Restrict question_bank to authenticated users only
...
-- Create a secure function to validate game answers server-side
-- This function will be called by the edge function with service role
CREATE OR REPLACE FUNCTION public.validate_game_submission(
  p_user_id uuid,
  p_unit_id uuid,
  p_passage_id uuid,
  p_answers jsonb, -- array of {question_id, answer_index}
...
BEGIN
  ...
  -- XP calculation: base XP from score + time bonus
...
-- Grant execute permission to authenticated users (will be called via edge function with service role)
GRANT EXECUTE ON FUNCTION public.validate_game_submission TO service_role;